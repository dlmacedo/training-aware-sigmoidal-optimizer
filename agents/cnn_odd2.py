...
class CNNAgent:
    ...
    def __init__(self, args):
        ...
        if self.args.loss.startswith("eml"):
            parameters = self.parameters_eml_plus(
        ...
        elif self.args.loss.startswith("xml"):
            parameters = self.parameters_xml_plus(self.args.loss.split("_")[8], float(self.args.loss.split("_")[9]))
            #print(parameters)
            self.optimizer = torch.optim.SGD(
                parameters,
                lr=self.args.original_learning_rate,
            ...
            #parameters = self.parameters_weight_decay()
            #print(parameters)
            self.optimizer = torch.optim.SGD(
                parameters,
                lr=self.args.original_learning_rate,
    ...
    def parameters_eml_plus(self, weight_decay, learn_rate, frozen_prototypes):
        #regular_parameters, no_weight_decay_parameters, classifier_scales_parameters = [], [], []
        regular_parameters, classifier_prototypes_parameters, classifier_scales_tilts_batchnorm_parameters = [], [], []
        print()
        for name, param in self.model.named_parameters():
            if not param.requires_grad:
                continue  # frozen weights
            if name == "classifier.weights":
                ...
                print("##########################################################")
                classifier_scales_tilts_batchnorm_parameters.append(param)
            elif name == "classifier.tilts":
                print("##########################################################")
                print("SPECIAL OPTIMIZATION SCALES, TILTS AND NORMALIZATION:", name)
                print("##########################################################")
                classifier_scales_tilts_batchnorm_parameters.append(param)
            elif name.startswith("classifier.normalization"):
                print("##########################################################")
                print("SPECIAL OPTIMIZATION SCALES, TILTS AND NORMALIZATION:", name)
                print("##########################################################")
                classifier_scales_tilts_batchnorm_parameters.append(param)
        ...
        if (weight_decay == "WD") and (not frozen_prototypes):
            ...
            print("WEIGHT DECAY SCALES, TILTS AND NORMALIZATION\n")
            dict_to_return = [
            {'params': regular_parameters},
            {'params': classifier_prototypes_parameters},
            {'params': classifier_scales_tilts_batchnorm_parameters, 'lr': learn_rate}]
        elif (weight_decay == "NO") and (not frozen_prototypes):
            print("\nLEARNABLE PROTOTYPES")
            print("NO WEIGHT DECAY SCALES, TILTS AND NORMALIZATION\n")
            dict_to_return = [
            {'params': regular_parameters},
            {'params': classifier_prototypes_parameters},
            {'params': classifier_scales_tilts_batchnorm_parameters, 'lr': learn_rate, 'weight_decay': 0.}]
    ...
    def train_validate(self):
        ...
        for self.epoch in range(1, self.args.epochs + 1):
            ...
            #train_loss, train_acc1, train_epoch_logits, train_epoch_metrics, train_epoch_entropies_per_classes = self.train_epoch()
            train_loss, train_acc1, train_epoch_logits, train_epoch_metrics = self.train_epoch()
            # Adjusting learning rate (if not using reduce on plateau)...
            self.scheduler.step()
            #valid_loss, valid_acc1, valid_epoch_logits, valid_epoch_metrics, valid_epoch_entropies_per_classes = self.validate_epoch()
            valid_loss, valid_acc1, valid_epoch_logits, valid_epoch_metrics = self.validate_epoch()
            ...
            #train_misc_stat = calculate_misc_stat(train_misc_stat, self.args.number_of_model_classes)
            #valid_misc_stat = calculate_misc_stat(valid_misc_stat, self.args.number_of_model_classes)
    ...
    def train_epoch(self):
        ...
        #misc_stat = {"intraclass": {}, "interclass": {}}
        #all_misc_stat = {}
        ...
        for batch_index, (inputs, targets) in enumerate(self.trainset_loader_for_train):
            ...
            # compute loss
            #loss, outputs, intra_logits, inter_logits, max_probabilities, entropies, entropies_per_classes = self.criterion(features, targets)
            #loss, outputs, intra_logits, inter_logits, max_probabilities, entropies = self.criterion(features, targets)
            loss, outputs, odd_outputs, intra_logits, inter_logits = self.criterion(features, targets)

            ##########################################################################
            max_probabilities = nn.Softmax(dim=1)(odd_outputs).max(dim=1)[0].tolist()
            entropies = utils.entropies_from_logits(odd_outputs).tolist()
            ##########################################################################
            ...
            if self.args.number_of_model_classes > 100:
                ...
                epoch_logits["intra"] += intra_logits
                epoch_logits["inter"] += inter_logits
            epoch_metrics["max_probs"] += max_probabilities
            epoch_metrics["entropies"] += entropies
            #for index in range(len(epoch_entropies_per_classes)):
            #    epoch_entropies_per_classes[index] += entropies_per_classes[index]
        ...
        return loss_meter.avg, accuracy_meter.value()[0], epoch_logits, epoch_metrics#, epoch_entropies_per_classes
    ...
    def odd_infer(self):
        ...
        for out_dist in out_dist_list:
            ...
            _, _, valid_epoch_logits, valid_epoch_metrics = self.validate_epoch()
            np.save(os.path.join(

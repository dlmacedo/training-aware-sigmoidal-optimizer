...
def get_curve(dir_name, stypes=['Baseline', 'Gaussian_LDA']):
    ...
    for stype in stypes:
        ...
        #known = np.loadtxt('{}/confidence_{}_In.txt'.format(dir_name, stype), delimiter='\n')
        #novel = np.loadtxt('{}/confidence_{}_Out.txt'.format(dir_name, stype), delimiter='\n')
        cifar = np.loadtxt('{}/confidence_{}_In.txt'.format(dir_name, stype), delimiter='\n')
        other = np.loadtxt('{}/confidence_{}_Out.txt'.format(dir_name, stype), delimiter='\n')
        #X = np.concatenate((cifar[:, score_key], other[:, score_key]))
        #Y = np.concatenate((np.full(cifar[:, score_key].shape, 1), np.full(other[:, score_key].shape, 0)))
        X = np.concatenate((cifar, other))
        Y = np.concatenate((np.full(cifar.shape, 1), np.full(other.shape, 0)))
        fpr, tpr, _ = roc_curve(Y, X)
        fpr_tpr95 = 100*fpr[(np.abs(tpr - 0.95)).argmin()]
        #der = 100*np.mim((1 - ((tpr + (1 - fpr))/2.0)))
        der = 100*np.min((1 - ((tpr + (1 - fpr))/2.0)))
        auc = 100*roc_auc_score(Y, X)
        aupr = 100*average_precision_score(Y, X)
        """
        ###########################################################################################
